...
function sanitizeGoodBad(good: number, bad: number): { good: number; bad: number } {
    const g = Number(good);
    const b = Number(bad);
    if (!Number.isFinite(g) || !Number.isFinite(b)) return { good: 0, bad: 1 };
    if (g === b) return { good: g, bad: g + 1e-6 };
...
export function scoreDecreasing1Sided(x: number, good: number, bad: number): number {
    ...
    if (x <= g) return 100;
    if (x < b) return clamp100((100 * (b - x)) / (b - g));
    return 0;
}

function sanitizeTrapezoid(a: number, b: number, c: number, d: number): { a: number; b: number; c: number; d: number } {
    const aa = Number(a);
    const bb = Number(b);
    const cc = Number(c);
    const dd = Number(d);
    if ([aa, bb, cc, dd].some((v) => !Number.isFinite(v))) return { a: 0, b: 1, c: 1, d: 2 };
...
function scoreTrapezoid2Sided(x: number, a: number, b: number, c: number, d: number): number {
